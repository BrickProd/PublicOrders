...
namespace PublicOrders.Processors.Internet
{
    class _223WinnerChoice
    {
        ...
        public ResultType_enum Work(Order order, InternetRequestEngine internetRequestExplorer,
                               HtmlAgilityPack.HtmlNode winnerProtocolNode, LotSearched_delegate lotSearched_delegate,
                               out string winnerChoiceMessage, out bool isWinner)
        {
            try
            {
                isWinner = false;
                ...
                text = @"http://zakupki.gov.ru/223/purchase/public/purchase/protocol/rkz/application/winner-choice.html?noticeInfoId=&protocolInfoId=" + protocolID + "&mode=view";
                doc = internetRequestExplorer.GetHtmlDoc(text);
                string checkMessage = "";
                ResultType_enum resultTypeCheck = Globals.CheckDocResult(doc, out checkMessage);
                if (resultTypeCheck != ResultType_enum.Done)
                {
                    winnerChoiceMessage = checkMessage;
                ...
                /*if ((!allHasName) && (hasWinners))
                {
                    ...
                    if (lotsTemp.Count > 0)
                    {
                        ...
                        resultTypeCheck = Globals_DLL.CheckDocResult(doc, out checkMessage);
                        ...
                        if (lotNamesColl == null)
                        {
                            ...
                            if (lotNamesColl == null) return ResultType.NotSearch;
                        }
                        ...
                        foreach (HtmlAgilityPack.HtmlNode lotNameNode in lotNamesColl)
                        {
                            try
                            {
                                lotsTemp[lotNameNum].name = Globals_DLL.DecodeInternetSymbs(lotNameNode.InnerText.Trim());
                            }
                            catch
                            {
                                continue;
                                ...
                                if (customersColl != null)
                                {
                                    ...
                                    foreach (HtmlAgilityPack.HtmlNode customerNode in customersColl)
                                    {
                                        ...
                                        foreach (HtmlAgilityPack.HtmlNode customerAttributeNode in customerAttributesColl)
                                        {
                                            ...
                                            {
                                                case (2):
                                                    string nameTmp = Globals_DLL.DecodeInternetSymbs(customerAttributeNode.InnerText.Trim());
                                                ...
                                                case (4):
                                                    if (customerAttributeNode.InnerText.Trim() != "")
                                                    {
                                                        string priceTypePriceLot = Globals_DLL.DecodeInternetSymbs(customerAttributeNode.InnerText.Trim());
                                                        if (priceTypePriceLot.IndexOf(' ') > -1)
                                                        {
                                                            _223lotDocumentPrice = Globals_DLL.DecodeInternetSymbs(priceTypePriceLot.Substring(0, priceTypePriceLot.IndexOf(' ')).Trim());
                                                        }
                                                    }
                                                    break;
                                            }
                                        ...
                                        if (itsWinner)
                                        {
                                            ...
                                            lotsTemp[lotPriceDateNum].documentDateTime = _223lotDocumentDate;
                                            break;
                                        }
                                        else
                                        {
                                            _223lotDocumentPrice = "";
                                            _223lotDocumentDate = "";
                                        }
                                    }
                                }
                ...
                if (hasWinners)
                {
                    foreach (Winner winner in winnersTemp)
                    {
                        if ((winner.Name != "") && (winner.Lot.Name != ""))
                        {
                            mvm.wc.Lots.Add(winner.Lot);
                            mvm.wc.SaveChanges();
            ...
            {
                isWinner = false;
                winnerChoiceMessage = ex.Message + ' ' + ex.StackTrace;
